...
class CrossAttention(nn.Module):
    ...
    def forward(self, x,y, attn_kv=None, mask=None):
        B_, N, C = x.shape
        q, k, v = self.qkv(x, attn_kv)
        q_y, k_y, v_y = self.qkv(y, attn_kv)
        q = q * self.scale
...
class RSA(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        list_up = [up_0, up_1, up_2, up_3]
        # Output Projection
        y = self.output_proj(deconv3)
        return (x + y if self.dd_in == 3 else y), list_up
...
class ISA(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        # Decoder
        self.upsample_0 = upsample(embed_dim * 16, embed_dim * 8)
        self.attn_0 = CrossAttention(
            embed_dim * 8, win_size=to_2tuple(win_size), num_heads=num_heads[4],
        ...
        self.attn_1 = CrossAttention(
            embed_dim * 4, win_size=to_2tuple(win_size), num_heads=num_heads[5],
            qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
            token_projection='linear')
        self.decoderlayer_1 = BasicUformerLayer(dim=embed_dim * 8,
                                                output_dim=embed_dim * 8,
        ...
        self.attn_2 = CrossAttention(
            embed_dim * 2, win_size=to_2tuple(win_size), num_heads=num_heads[6],
            qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
            token_projection='linear')
        self.decoderlayer_2 = BasicUformerLayer(dim=embed_dim * 4,
                                                output_dim=embed_dim * 4,
                                                ...
                                                modulator=modulator, cross_modulator=cross_modulator)
        self.upsample_3 = upsample(embed_dim * 4, embed_dim)
        self.attn_3 = CrossAttention(
            embed_dim, win_size=to_2tuple(win_size), num_heads=num_heads[7],
            qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
            token_projection='linear')
        self.decoderlayer_3 = BasicUformerLayer(dim=embed_dim * 2,
                                                output_dim=embed_dim * 2,
    ...
    def forward(self, x, list, mask=None):
        ...
        # Decoder
        up0 = self.upsample_0(conv4)
        attention_0 = self.attn_0(list[0],up0)
        up0 = up0 + attention_0
        deconv0 = torch.cat([up0, conv3], -1)
        deconv0 = self.decoderlayer_0(deconv0, mask=mask)

        up1 = self.upsample_1(deconv0)
        # attention_1 = self.attn_1(list[1],up1)
        # up1 = up1 + attention_1
        deconv1 = torch.cat([up1, conv2], -1)
        deconv1 = self.decoderlayer_1(deconv1, mask=mask)

        up2 = self.upsample_2(deconv1)
        # attention_2 = self.attn_2(list[2], up2)
        # up2 = up2 + attention_2
        deconv2 = torch.cat([up2, conv1], -1)
        deconv2 = self.decoderlayer_2(deconv2, mask=mask)

        up3 = self.upsample_3(deconv2)
        # attention_3 = self.attn_3(list[3], up3)
        # up3 = up3 + attention_3
        deconv3 = torch.cat([up3, conv0], -1)
        deconv3 = self.decoderlayer_3(deconv3, mask=mask)
